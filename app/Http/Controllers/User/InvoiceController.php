...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth()->user()->phone === null) {
            return redirect()->route('profile.edit')->with(['number' => 'No handphone tidak boleh kosong']);
        }
        if ($request->ajax()) {
        ...
        return view('pages.users.topup.index');
    }
    ...
    {
        $html = '<form id="createInvoice" action="'. route("invoce.store") .'" method="post">'. csrf_field() .'
            <div class="row">
                <div class="col-sm-12">
                ...
                <div class="col-sm-12">
                    <div class="list-method" style="height: 300px; overflow-x: hidden; overflow-y: auto;">
                        ...
                        foreach ($channels as $channel) {
                            ...
                            $html .= '
                                <input type="hidden" class="form-control" name="amount" readonly value="'. $data .'">
                                <div class="light-background border b-r-8 border-primary h-100 text-center">
                                    <div class="card-body">
                                        <img width="100" src="' . asset("payment/". $channel->logo) . '">
                                        <p class="f-light mt-3 mb-0">'. $channel->name .'</p>
                                        ...
                                        $html .= '</p>
                                        <button type="button" class="btn btn-outline-primary bank-btn" data-bank="'. $channel->code .'">Bayar</button>
                                    </div>
                                </div>
                            </div>';
                        }
    ...
    public function success(Request $request)
    {
        ...
        if(isset($response['status']) && $response['status'] == 'COMPLETED') {
            $invoice = Invoice::where('ref_id', $response['id'])
